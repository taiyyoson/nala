...
from utils.unified_storage import convert_session1_to_unified, save_unified_session
...
class Session1Manager:
    ...
    def __init__(
        self,
        program_info_file: str = "program_info.txt",
        llm_client=None,
        uid: str = None,
        debug=True,
    ):
    ...
    def process_user_input(
        ...
        if self.state == SessionState.GREETINGS:
            if user_input.strip() == "[START_SESSION]":
                ...
                result[
                    "context"
                ] = "First message. Introduce yourself as Nala and ask for their name."
                return result
            ...
            result[
                "context"
            ] = f"User's name: {self.session_data.get('user_name', 'Not provided')}\n\nExplain program details."

        elif self.state == SessionState.PROGRAM_DETAILS:
            if any(
                word in user_lower
                for word in ["yes", "yeah", "yep", "sure", "i do", "i have"]
            ):
                ...
                word in user_lower
                for word in ["no", "nope", "nah", "don't", "dont", "no questions"]
            ):
            ...
            if no_more:
                ...
                result[
                    "context"
                ] = "User has no more questions. Transition to discovery phase."
            elif is_asking:
                result["next_state"] = SessionState.ANSWERING_QUESTION
                result["trigger_rag"] = False
                result[
            ...
            if self.session_data.get("last_coach_response"):
                ...
                coach_asking_about_questions = any(
                    prompt in coach_lower for prompt in question_prompts
                )

            if check_affirmative(user_input):
                if coach_asking_about_questions and any(
                    word in self.session_data.get("last_coach_response", "").lower()
                    for word in ["does that help", "clarify", "answer your question"]
                ):
                    result["next_state"] = SessionState.PROMPT_TALK_ABOUT_SELF
                    result[
                        "context"
                    ] = "User's question was answered. Transition to discovery phase."
                    ...
                    result[
                        "context"
                    ] = "User has questions. Ask what they'd like to know."

            if check_negative(user_input):
                result["next_state"] = SessionState.PROMPT_TALK_ABOUT_SELF
                result[
                    "context"
                ] = "User has no questions. Transition to discovery phase."
            else:
            ...
            if has_more:
                ...
                result[
                    "context"
                ] = "User ready to start. Transition to discovery phase."
            else:
                result["next_state"] = SessionState.AWAITING_YES_NO
                result[
                    "context"
                ] = "Ask if they have any other questions about the program before starting."

        elif self.state == SessionState.PROMPT_TALK_ABOUT_SELF:
            result["next_state"] = SessionState.GETTING_TO_KNOW_YOU
            result[
                "context"
            ] = "Transition to discovery. Explain you'll ask questions to know them."
            ...
            if goal_text and min_discovery_complete:
                ...
                result[
                    "context"
                ] = f"""User stated a goal after discovery!
Discovery info:
...
Now transition to goal setting. Ask: "What specific health or wellness goal would you like to focus on?" """
                ...
                result[
                    "context"
                ] = f"""Discovery - next question: {next_topic}
...
Explain what's missing and guide refinement."""
                ...
                result[
                    "context"
                ] = "Not a clear goal. Ask them to describe what they'd like to achieve."
            ...
            if is_smart:
                ...
                result[
                    "context"
                ] = f"Goal is SMART! Celebrate and ask for confidence level."
            else:
...
Guide them to make it more SMART. Be specific about what's missing."""
            ...
            if self.session_data.get("last_coach_response"):
                refined_from_coach = extract_refined_goal_from_coach_response(
                    self.session_data["last_coach_response"], user_input
                )
                if refined_from_coach:
                    self._log_debug(
                        f"Extracted refined goal from coach: '{refined_from_coach}'"
                    )
            ...
            if refined_from_coach and user_confirming and len(user_input.split()) <= 3:
                self._log_debug(
                    f"User confirmed coach's refined goal: '{refined_from_coach}'"
                )
                ...
                if smart_eval["is_smart"]:
                    ...
                    result[
                        "context"
                    ] = f"Refined goal is SMART: '{refined_from_coach}'\nAsk for confidence level (1-10 scale)."
                else:
                    result[
                        "context"
                    ] = f"Goal refined to: '{refined_from_coach}' but still needs work.\nContinue helping them make it more specific."
                return result
            ...
            if self.session_data.get("last_coach_response") and not user_confirming:
                ...
                if refined_goal:
                    ...
                    if smart_eval["is_smart"]:
                        ...
                        result[
                            "context"
                        ] = f"Refined goal is SMART: '{refined_goal}'\nAsk for confidence level (1-10 scale)."
                        return result
                    else:
                        result[
                            "context"
                        ] = f"Goal refined to: '{refined_goal}' but still needs work.\nContinue helping them make it more specific."
                        return result

            # Use utility to check if coach accepted goal
            coach_accepted = False
            if self.session_data.get("last_coach_response"):
                coach_accepted = check_coach_goal_acceptance(
            ...
            is_moving_forward = any(
                phrase in user_lower for phrase in moving_forward_phrases
            )

            if (is_moving_forward or coach_accepted) and self.session_data[
                "current_goal"
            ]:
                if not self.session_data["goal_details"]:
                    self._store_completed_goal(confidence=8)

                if is_moving_forward and any(
                    phrase in user_lower
                    for phrase in ["todo", "calendar", "reminder", "track", "note"]
                ):
                    result["next_state"] = SessionState.REMEMBER_GOAL
                    result[
                        "context"
                    ] = f"""User described tracking method: "{user_input}"
...
Acknowledge their tracking plan and ask if there's anything else before wrapping up."""
                    ...
                    result[
                        "context"
                    ] = f"""Goal accepted: "{self.session_data['current_goal']}"
Ask for confidence level (1-10 scale)."""

            elif looks_like_goal:
                is_question = any(
                    indicator in goal_candidate.lower()
                    for indicator in [
                        "?",
                        "how about",
                        "what if",
                ...
                ) or goal_candidate.endswith("?")

                if is_question:
                    result[
                        "context"
                    ] = "User asking a question. Answer and help them finalize the goal statement."
                    return result
                ...
                if smart_eval["is_smart"]:
                    ...
                    if self.llm_client:
                        ...
                        cleaned_goal = reword_goal_with_llm(
                            self.session_data["current_goal"], self.llm_client, context
                        )
                        self._log_debug(
                            f"Cleaned assembled goal from '{self.session_data['current_goal']}' to '{cleaned_goal}'"
                        )
                    ...
                    result[
                        "context"
                    ] = f"Refined goal is SMART: '{self.session_data['current_goal']}'\nAsk for confidence level."
                else:
                    missing = ", ".join(smart_eval["missing_criteria"])
                    ...
                    if attempts >= 4:
                        ...
                        result[
                            "context"
                        ] = f"After {attempts} attempts, move forward with: '{self.session_data['current_goal']}'\nAsk for confidence."
                    else:
                        result["next_state"] = SessionState.REFINE_GOAL
                        result[
                            "context"
                        ] = f"""Refinement attempt {attempts}.
Goal: "{self.session_data['current_goal']}"
...
Ask specific question to get the missing piece. What exactly do they need to add?"""
                ...
                if any(
                    phrase in user_lower
                    for phrase in ["stick with", "keep", "thats all", "that's all"]
                ):
                    if self.session_data["current_goal"]:
                        ...
                        result[
                            "context"
                        ] = f"User keeping: '{self.session_data['current_goal']}'. Check confidence."
                    else:
            ...
            if confidence:
                ...
                if confidence <= 7:
                    ...
                    result[
                        "context"
                    ] = f"Low confidence ({confidence}/10). Explore what would help increase it."
                else:
                    result["next_state"] = SessionState.HIGH_CONFIDENCE
                    result[
                        "context"
                    ] = f"High confidence ({confidence}/10). Move to asking about more goals."
            else:
                result["context"] = "Ask for numeric confidence (1-10)"

        elif self.state == SessionState.LOW_CONFIDENCE:
            if (
                "rework" in user_lower
                or "change" in user_lower
                or "different" in user_lower
            ):
                result["next_state"] = SessionState.GOALS
                result[
                    "context"
                ] = "User wants to rework goal. Ask what they'd like to focus on instead."
            else:
                result["next_state"] = SessionState.ASK_MORE_GOALS
                result[
                    "context"
                ] = "Acknowledge concerns. Move to asking about more goals."

        elif self.state == SessionState.HIGH_CONFIDENCE:
            result["next_state"] = SessionState.ASK_MORE_GOALS
            result[
                "context"
            ] = "High confidence confirmed. Ask if they want to set another goal."
            ...
            user_described_tracking = any(
                method in user_lower for method in tracking_methods
            )

            if user_described_tracking:
                self.session_data["tracking_method_discussed"] = True
                result["next_state"] = SessionState.CONFIRM_END_SESSION
                result[
                    "context"
                ] = f"""User described tracking method: "{user_input}"
Acknowledge their plan. Before ending, ask the user: 'Is there anything else you'd like to discuss before we wrap up today?' Be warm and genuine."""
                return result
            ...
            if self.session_data.get("last_coach_response"):
                ...
                coach_wrapped_up = any(
                    phrase in coach_lower for phrase in wrapping_phrases
                )
            ...
            if self.session_data.get("last_coach_response"):
                ...
                coach_asked_tracking = any(
                    phrase in coach_lower for phrase in tracking_questions
                )

            user_confirming_end = user_lower in [
                "are we done",
                "is that it",
                "are we finished",
                "ok",
            ...
            if wants_more and "focus" not in user_lower:
                ...
                result[
                    "context"
                ] = "Before ending, ask the user: 'Is there anything else you'd like to discuss before we wrap up today?' Be warm and genuine."
            elif no_more or "focus" in user_lower:
                # Check if tracking was already discussed
                tracking_discussed = self.session_data.get(
                    "tracking_method_discussed", False
                )
                ...
                if coach_asked_tracking and no_more:
                    ...
                    result[
                        "context"
                    ] = "User confirmed no more goals after tracking question. Before ending, ask the user: 'Is there anything else you'd like to discuss before we wrap up today?' Be warm and genuine."
                elif tracking_discussed or coach_wrapped_up:
                    result["next_state"] = SessionState.CONFIRM_END_SESSION
                    result[
                        "context"
                    ] = "No more goals and tracking discussed. Before ending, ask the user: 'Is there anything else you'd like to discuss before we wrap up today?' Be warm and genuine."
                else:
                    result["next_state"] = SessionState.REMEMBER_GOAL
                    result[
                        "context"
                    ] = "No more goals. Ask how they'll track/remember their goal."
            else:
                result[
                    "context"
                ] = "Ask user to clarify: would they like to set another goal or focus on this one?"
            ...
            if self.session_data.get("last_coach_response"):
                ...
                coach_asking_wrap_up = any(
                    phrase in coach_lower for phrase in wrap_up_questions
                )
            ...
            if self.session_data.get("last_coach_response"):
                ...
                coach_said_goodbye = any(
                    phrase in coach_lower for phrase in goodbye_phrases
                )

            # Check if user is ready to end
            user_ready_to_end = any(
                phrase in user_lower
                for phrase in [
                    "no",
                    "nope",
            ...
            if coach_said_goodbye or (coach_asking_wrap_up and user_ready_to_end):
                ...
                result[
                    "context"
                ] = f"""Session wrapping up naturally.
...
Before ending, ask the user: 'Is there anything else you'd like to discuss before we wrap up today?' Be warm and genuine."""
            # If tracking described but no wrap-up yet, ask about anything else
                ...
                result[
                    "context"
                ] = "Acknowledge tracking plan and ask: 'Is there anything else you'd like to talk about before we wrap up today?'"
            # If no tracking mentioned yet, ask about it
            elif not described_tracking:
                result[
                    "context"
                ] = "Ask about their tracking plan. How will they remember their goal?"
            else:
                result["context"] = "Acknowledge and prepare to wrap up session."

        elif self.state == SessionState.CONFIRM_END_SESSION:
            # Confirmation gate before truly ending the session
            confirm_done_phrases = [
                "yes",
                "yep",
                "yeah",
                "yup",
            ...
            user_is_confirming_done = any(
                phrase in user_lower for phrase in confirm_done_phrases
            )

            # Check if user said something substantive (more than just a short confirmation)
            substantive_input = len(user_input.split()) > 4 or (
                not user_is_confirming_done and len(user_input.strip()) > 0
            )

            if substantive_input:
                # User has more to discuss - route back to ASK_MORE_GOALS
                result["next_state"] = SessionState.ASK_MORE_GOALS
                result[
                    "context"
                ] = f"""The user said something substantive when asked if they wanted to wrap up: "{user_input}"
Address what the user said. They may have a new topic, question, or goal to discuss."""
            else:
                # User confirmed they're done - proceed to actual end
                result["next_state"] = SessionState.END_SESSION
                result[
                    "context"
                ] = "User confirmed they are done. Provide a warm, brief goodbye and confirm next session is in 1 week."

        elif self.state == SessionState.END_SESSION:
            # Session is complete - stay in END_SESSION
            result[
                "context"
            ] = "Session complete. Provide brief, warm confirmation if needed, but session is ending."

            if not self.session_data.get("goal_details"):
                result["next_state"] = SessionState.GOALS
                result[
                    "context"
                ] = "ERROR: No goals stored. Let's make sure we have a clear goal before ending."
    ...
    def _store_completed_goal(self, confidence: int = None):
        ...
        if was_stored:
            ...
            if self.session_data["goal_details"]:
                self.session_data["current_goal"] = self.session_data["goal_details"][
                    -1
                ]["goal"]
                self._log_debug(
                    f"Stored reworded goal: {self.session_data['current_goal']}"
                )
    ...
    def save_session(self, filename: str = None, conversation_history: list = None):
        ...
        discovery = {
            ...
            "free_time_activities": self.session_data["discovery"].get(
        ...
        session_metadata = {
            ...
            "current_goal": self.session_data.get("current_goal"),
            "program_questions_asked": self.session_data.get(
                "program_questions_asked", []
            ),
            "tracking_method_discussed": self.session_data.get(
                "tracking_method_discussed", False
            ),
        }
    ...
    def load_session(self, filename: str):
        ...
        self.session_data["program_questions_asked"] = metadata.get(
            "program_questions_asked", []
        )
        self.session_data["tracking_method_discussed"] = metadata.get(
            "tracking_method_discussed", False
        )
