...
def get_user_progress(
    ...
    try:
        ...
        progress_list = (
            db.query(SessionProgress)
            .filter_by(user_id=auth_user_id)
...
def get_session_data(
    ...
    Args:
        user_id (str): Firebase UID of the user (path param, overridden by token)
        session_number (int): Session number (1–4)
    ...
    try:
        ...
        data = load_session_from_db(auth_user_id, session_number)

        if not data:
            raise HTTPException(
                status_code=404,
                detail=f"No data found for session {session_number}",
            )
...
def get_latest_session(
    ...
    try:
        ...
        data = get_latest_session_for_user(auth_user_id)
